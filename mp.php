<!doctype html>
<html lang="zh-Hant" class="no-js">
<head>
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>司法院全球資訊網</title>
    <!--HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries [if lt IE 9]>
    <script src="js/html5shiv.js"></script>
    <script src="js/respond.min.js"></script>
  <![endif]-->
  <!-- slick css-->
  <link rel="stylesheet" type="text/css" href="vendor/slick/slick.css" />
  <link rel="stylesheet" type="text/css" href="vendor/slick/slick-theme.css" />
  <!-- hyUI css -->
  <link rel="stylesheet" href="css/judicial.css">
</head>

<body>
  <!-- 直接跳主內容區 -->
  <a class="goCenter" href="#center" tabindex="1">按Enter到主內容區</a>
  <!-- wrapper Start -->
  <div class="wrapper">
    <!-- header Start -->
    <?php require_once('include/header.html'); ?>
    <!-- header End -->
    <!-- main Start -->
    <div id="center" class="main mp">
      <div class="container">
        <a class="accesskey" href="#aC" id="aC" accesskey="C" title="主要內容區">:::</a>
        <!-- 首頁輪播 -->
        <section class="mpSlider">
          <div class="img-container">
            <a href="#" title="">
              <img src="images/indexbanner1.jpg" alt="圖片說明1" class="cover">
            </a>
          </div>
          <div class="img-container">
            <a href="#" title="">
              <img src="images/banner_newyear.jpg" alt="圖片說明2" class="cover">
            </a>
          </div>
          <div class="img-container">
            <a href="#" title="">
              <img src="images/banner_judicial.jpg" alt="圖片說明3" class="cover">
            </a>
          </div>
          <div class="img-container">
            <a href="#" title="">
              <img src="images/banner_draginboat.jpg" alt="圖片說明4" class="cover">
            </a>
          </div>
        </section>
        <!-- 影片只有一則時css不需要加 video1和 video2 -->
        <div class="mpvideo video1">
          <div class="video-container">
            <iframe width="560" height="315" src="https://www.youtube.com/embed/e1-0l0of8h0" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
          </div>
          <a href="" class="close"></a>
        </div>
        <!-- 影片只有一則時css不需要加 video1和 video2 -->
        <div class="mpvideo video2">
          <div class="video-container">
            <iframe width="560" height="315" src="https://www.youtube.com/embed/e1-0l0of8h0" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
          </div>
          <a href="" class="close"></a>
        </div>
        <!-- 報名專區 -->
        <section class="apply_block">
          <h2 class="blkTitle">報名專區</h2>
          <ul class="apply_list">
            <li>
              <span>活動日期：108-12-08起</span>
              <a href="">司法院大法官108年度學術研討會 (主題：憲法訴訟的新時代)司法院大法官108年度學術研討會 (主題：憲法訴訟的新時代)</a>
            </li>
            <li>
              <span>活動日期：108-12-06</span>
              <a href="">司法院所屬各機關法官助理管理系統宣導會議</a>
            </li>
            <li>
              <span>活動日期：108-12-02</span>
              <a href="">司法院108年度強化司法業務暨宣導之官網建置採購案上稿教育訓練司法院108年度強化司法業務暨宣導之官網建置採購案上稿教育訓練</a>
            </li>
          </ul>
          <div class="more"><a href="">更多報名訊息</a></div>
          <div class="apply_bg">
            <img src="images/apply_bg.jpg" alt="">
          </div>
        </section>
        <div class="course">
          <h2 class="blkTitle">近期活動<span>Activities</span></h2>
          <ul>
            <li>
              <a href="">
                <span class="course_date"><em>12</em>27</span>
                <span class="course_title">從創新研究到臨床應用繼續教育</span>
                <span class="course_pic"><img src="images/news1.jpg" alt=""></span>
              </a>
            </li>
            <li>
              <a href="">
                <span class="course_date"><em>12</em>27</span>
                <span class="course_title">從創新研究到臨床應用繼續教育從創新研究到臨床應用繼續教育從創新研究到臨床應用繼續教育從創新研究到臨床應用繼續教育</span>
                <span class="course_pic"><img src="images/news1.jpg" alt=""></span>
              </a>
            </li>
            <li>
              <a href="">
                <span class="course_date"><em>12</em>27</span>
                <span class="course_title">從創新研究到臨床應用繼續教育</span>
                <span class="course_pic"><img src="images/news1.jpg" alt=""></span>
              </a>
            </li>
            <li>
              <a href="">
                <span class="course_date"><em>12</em>27</span>
                <span class="course_title">從創新研究到臨床應用繼續教育</span>
                <span class="course_pic"><img src="images/news1.jpg" alt=""></span>
              </a>
            </li>
            <li>
              <a href="">
                <span class="course_date"><em>12</em>27</span>
                <span class="course_title">從創新研究到臨床應用繼續教育</span>
                <span class="course_pic"><img src="images/news1.jpg" alt=""></span>
              </a>
            </li>
          </ul>
        </div>
        <section class="center_block">
          <div class="link">
            <ul>
              <li><a href="">釋憲訊息</a></li>
              <li><a href="">行事曆</a></li>
            </ul>
          </div>
          <h2 class="blkTitle">司法新聞<span>Judicial News</span></h2>
          <div class="news">
            <div class="tabSet">
              <div class="tabs">
                <h3 class="tabItem active"><a href="#">本院新聞稿</a></h3>
                <div class="tabContent">
                  <ul class="piclist">
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title"><em>力排眾議司法院修刑事訴訟法排除測謊當證據力排眾議司法院修刑事訴訟法排除測謊當證據</em></span>
                        <span class="news_pic">
                          <img src="images/news1.jpg" alt="">
                        </span>
                      </a>
                    </li>
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title"><em>力排眾議司法院修刑事訴訟法排除測謊</em></span>
                        <span class="news_pic">
                          <!-- <img src="images/news2.jpg" alt=""> -->
                          <img src="https://via.placeholder.com/728x90.png" alt="">
                        </span>
                      </a>
                    </li>
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title"><em>力排眾議司法院修刑事訴訟法排除測謊當證據力排眾議司法院修刑事訴訟法排除測謊當證據</em></span>
                        <span class="news_pic">
                          <!-- <img src="images/news3.jpg" alt=""> -->
                          <img src="https://via.placeholder.com/200x500.png" alt="">
                        </span>
                      </a>
                    </li>
                  </ul> 
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                  </ul>
                  <div class="more"><a href="">更多本院新聞稿</a></div>
                </div>
                <h3 class="tabItem"><a href="#">各法院新聞</a></h3>
                <div class="tabContent">
                  <ul class="piclist">
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title"><em>力排眾議司法院修刑事訴訟法排除測謊當證據力排眾議司法院修刑事訴訟法排除測謊當證據</em></span>
                        <span class="news_pic">
                          <img src="images/news1.jpg" alt="">
                        </span>
                      </a>
                    </li>
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title"><em>力排眾議司法院修刑事訴訟法排除測謊當證據力排眾議司法院修刑事訴訟法排除測謊當證據</em></span>
                        <span class="news_pic">
                          <img src="images/news2.jpg" alt="">
                        </span>
                      </a>
                    </li>
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title"><em>力排眾議司法院修刑事訴訟法排除測謊當證據力排眾議司法院修刑事訴訟法排除測謊當證據</em></span>
                        <span class="news_pic">
                          <img src="images/news3.jpg" alt="">
                        </span>
                      </a>
                    </li>
                  </ul> 
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                  </ul>
                  <div class="more"><a href="">更多各法院新聞</a></div>
                </div>
                <h3 class="tabItem"><a href="#">即時新聞澄清</a></h3>
                <div class="tabContent">
                  <ul class="piclist">
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title">力排眾議司法院修刑事訴訟法排除測謊當證據力排眾議司法院修刑事訴訟法排除測謊當證據</span>
                        <span class="news_pic">
                          <img src="images/news1.jpg" alt="">
                        </span>
                      </a>
                    </li>
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title">本院賡續辦理「破壞司法信譽案件」專案清查</span>
                        <span class="news_pic">
                          <img src="images/news2.jpg" alt="">
                        </span>
                      </a>
                    </li>
                    <li>
                      <a href="#">
                        <span class="news_date">108.05.31</span>
                        <span class="news_dep">統計處</span>
                        <span class="news_title">本院108年「司法輿情現況調查」結果出爐新聞稿</span>
                        <span class="news_pic">
                          <img src="images/news3.jpg" alt="">
                        </span>
                      </a>
                    </li>
                  </ul> 
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                  </ul>
                  <div class="more"><a href="">更多即時新聞澄清</a></div>
                </div>
                <h3 class="tabItem"><a href="#">法規異動</a></h3>
                <div class="tabContent">
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li> 
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>   
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>           
                  </ul> 
                  <div class="more"><a href="">更多法規異動</a></div>
                </div>
                <h3 class="tabItem"><a href="#">徵人啟事</a></h3>
                <div class="tabContent">
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li> 
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>   
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>           
                  </ul>
                  <div class="more"><a href="">更多徵人啟事</a></div>
                </div>
                <h3 class="tabItem"><a href="#">採購資訊</a></h3>
                <div class="tabContent">
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li> 
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>   
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>           
                  </ul> 
                  <div class="more"><a href="">更多採購資訊</a></div>
                </div>
                <h3 class="tabItem"><a href="#">其他資訊</a></h3>
                <div class="tabContent">
                  <ul class="textlist">
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li> 
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院賡續辦理「破壞司法信譽案件」專案清查，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心，展現維護民眾訴訟權益並強化司法清廉與公正形象之決心</a></span>
                      <span class="news_dep"><a href="#">政風處</a></span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院108年「司法輿情現況調查」結果出爐新聞稿</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">公務員懲戒委員會108年度澄字第3532號管中閔懲戒案開庭之旁聽事宜</a></span>
                      <span class="news_dep">公務員懲戒委員會</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">本院於本（108）年7月22日至8月23日委託循證民調有限公司，敬請民眾配合。</a></span>
                      <span class="news_dep">統計處</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>   
                    <li>
                      <span class="news_date">108.05.31</span>
                      <span class="news_title"><a href="#">宜蘭地院「逗陣繞法院」宜蘭縣縣長偕廉政小小志工共同體驗!.</a></span>
                      <span class="news_dep">宜蘭地方法院</span>
                    </li>           
                  </ul>
                  <div class="more"><a href="">更多其他資訊</a></div>
                </div>
              </div>
            </div>
          </div>
          <!-- 熱門服務 -->
          <div class="service">
            <h2 class="blkTitle">熱門服務</h2>
            <div class="serviceSlider">
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_dictionary.png" alt="">
                  <span class="icon_title">裁判書用語辭典資料庫查詢系統</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_lawhouse.png" alt="">
                  <span class="icon_title">法拍屋公告</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_court.png" alt="">
                  <span class="icon_title">開庭進度查詢</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_judgment.png" alt="">
                  <span class="icon_title">判決書查詢</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_dictionary.png" alt="">
                  <span class="icon_title">裁判書用語辭典資料庫查詢系統</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_lawhouse.png" alt="">
                  <span class="icon_title">法拍屋公告</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_court.png" alt="">
                  <span class="icon_title">開庭進度查詢</span>
                </a>
              </div>
              <div class="serviceBlk">
                <a href="#" title="">
                  <img src="images/icon_judgment.png" alt="">
                  <span class="icon_title">判決書查詢</span>
                </a>
              </div>
            </div>
          </div>
        </section>
        <!-- 司法業務類型 -->
        <section class="type_block">
          <h2 class="blkTitle">司法業務類型<span>Types of Judicial Service</span></h2>
          <div class="share">
            <ul>
              <li><a href="#"><img src="images/basic/icon_facebook.png" alt="facebook"></a></li>
              <li><a href="#"><img src="images/basic/icon_twitter.png" alt="twitter"></a></li>
              <li><a href="#"><img src="images/basic/icon_line2.png" alt="line"></a></li>
              <li><a href="#"><img src="images/basic/icon_plurk.png" alt="plurk"></a></li>
            </ul>
          </div>
          <div class="type">
            <div class="tabSet">
              <div class="tabs">
                <h3 class="tabItem active"><a href="#">民事</a></h3>
                <div class="tabContent">
                  <a href="">
                    <picture>
                      <!-- 大圖 -->
                      <source media="(min-width: 576px)" srcset="images/民事a.jpg">
                        <!-- 小圖 -->
                        <source media="(max-width: 575px)" srcset="images/民事b.jpg">
                          <!-- 大圖 -->
                          <img src="images/民事a.jpg">
                          <!-- 無障礙 -->
                          <noscript><img src="images/民事a.jpg"></noscript>
                        </picture>
                      </a>
                      <div class="more"><a href="">暸解更多</a></div>
                    </div>
                    <h3 class="tabItem"><a href="#">勞動<span>NEW!</span></a></h3>
                    <div class="tabContent">
                      <picture>
                        <source media="(min-width: 576px)" srcset="images/勞動a.jpg">
                          <source media="(max-width: 575px)" srcset="images/勞動b.jpg">
                            <img src="images/勞動a.jpg">
                            <noscript><img src="images/勞動a.jpg"></noscript>
                          </picture>
                          <div class="more"><a href="">暸解更多</a></div>
                        </div>
                        <h3 class="tabItem"><a href="#">刑事</a></h3>
                        <div class="tabContent">
                          <picture>
                            <source media="(min-width: 576px)" srcset="images/刑事a.jpg">
                              <source media="(max-width: 575px)" srcset="images/刑事b.jpg">
                                <img src="images/刑事a.jpg">
                                <noscript><img src="images/刑事a.jpg"></noscript>
                              </picture>
                              <div class="more"><a href="">暸解更多</a></div>
                            </div>
                            <h3 class="tabItem"><a href="#">少年</a></h3>
                            <div class="tabContent">
                              <picture>
                                <source media="(min-width: 576px)" srcset="images/少年a.jpg">
                                  <source media="(max-width: 575px)" srcset="images/少年b.jpg">
                                    <img src="images/少年a.jpg">
                                    <noscript><img src="images/少年a.jpg"></noscript>
                                  </picture>
                                  <div class="more"><a href="">暸解更多</a></div>
                                </div>
                                <h3 class="tabItem"><a href="#">家事</a></h3>
                                <div class="tabContent">
                                  <picture>
                                    <source media="(min-width: 576px)" srcset="images/家事a.jpg">
                                      <source media="(max-width: 575px)" srcset="images/家事b.jpg">
                                        <img src="images/家事a.jpg">
                                        <noscript><img src="images/家事a.jpg"></noscript>
                                      </picture>
                                      <div class="more"><a href="">暸解更多</a></div>
                                    </div>
                                    <h3 class="tabItem"><a href="#">行政</a></h3>
                                    <div class="tabContent">
                                      <picture>
                                        <source media="(min-width: 576px)" srcset="images/行政a.jpg">
                                          <source media="(max-width: 575px)" srcset="images/行政b.jpg">
                                            <img src="images/行政a.jpg">
                                            <noscript><img src="images/行政a.jpg"></noscript>
                                          </picture>
                                          <div class="more"><a href="">暸解更多</a></div>
                                        </div>
                                        <h3 class="tabItem"><a href="#">智財</a></h3>
                                        <div class="tabContent">
                                          <picture>
                                            <source media="(min-width: 576px)" srcset="images/智財a.jpg">
                                              <source media="(max-width: 575px)" srcset="images/智財b.jpg">
                                                <img src="images/智財a.jpg">
                                                <noscript><img src="images/智財a.jpg"></noscript>
                                              </picture>
                                              <div class="more"><a href="">暸解更多</a></div>
                                            </div>
                                            <h3 class="tabItem"><a href="#">商事</a></h3>
                                            <div class="tabContent">
                                              <picture>
                                                <source media="(min-width: 576px)" srcset="images/商事a.jpg">
                                                  <source media="(max-width: 575px)" srcset="images/商事b.jpg">
                                                    <img src="images/商事a.jpg">
                                                    <noscript><img src="images/商事a.jpg"></noscript>
                                                  </picture>
                                                  <div class="more"><a href="">暸解更多</a></div>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </section>
                                        <!-- 司法改革 -->
                                        <section class="reform_block">
                                          <h2 class="blkTitle">司法改革<span>Judicial Reform</span></h2>
                                          <div class="reform">
                                            <div class="tabSet">
                                              <section class="tabs">
                                                <h3 class="tabItem active"><a href="#">國民參與審判</a></h3>
                                                <div class="tabContent">
                                                  <h4>國民法官一起審判</h4>
                                                  <div class="reform_date">國民參與審判 推動中</div>
                                                  <div class="content">
                                                    <!-- 如果是兩欄加left -->
                                                    <div class="reform_pic left">
                                                      <a href="">
                                                        <span class="reform_caption">讓完全沒有審判專業知識、經驗的一般國民加入審判程序，參與聽訟、問案及最後判決形成的過程，使司法審判更透明讓完全沒有審判專業知識、經驗的一般國民加入審判程序，參與聽訟、問案及最後判決形成的過程，使司法審判更透明</span>
                                                        <img src="images/reform.jpg" alt="">
                                                      </a>
                                                    </div>
                                                    <!-- 右欄 -->
                                                    <div class="right_block">
                                                      <!-- 遊戲體驗 加game -->
                                                      <div class="block game">
                                                        <!-- 標題 -->
                                                        <div class="title">國民法官遊戲體驗</div>
                                                        <ul>
                                                          <li>
                                                            <a href="">
                                                              <span class="pic"><img src="images/game1.png" alt=""></span>
                                                              <span class="text">初體驗</span>
                                                            </a>
                                                          </li>
                                                          <li>
                                                            <a href="">
                                                              <span class="pic"><img src="images/game2.png" alt=""></span>
                                                              <span class="text">初體驗</span>
                                                            </a>
                                                          </li>
                                                        </ul>
                                                      </div>
                                                    </div>
                                                  </div>
                                                  <div class="reform_link"><a href="#">司法院與行政院司法改革進度追蹤資訊平台</a></div>
                                                </div>
                                                <h3 class="tabItem"><a href="#">勞動事件法</a></h3>
                                                <div class="tabContent">
                                                  <h4>兩張圖就是ul li</h4>
                                                  <div class="reform_date">勞動事件法 推動中</div>
                                                  <div class="content">
                                                    <div class="reform_pic left">
                                                      <a href="">
                                                        <span class="reform_caption">讓完全沒有審判專業知識、經驗的一般國民加入審判程序，參與聽訟、問案及最後判決形成的過程，使司法審判更透明...</span>
                                                        <img src="images/reform.jpg" alt="">
                                                      </a>
                                                    </div>
                                                    <div class="right_block">
                                                      <div class="block">
                                                        <!-- 兩張圖就是ul li -->
                                                        <ul>
                                                          <li>
                                                            <a href="">
                                                              <span class="pic">
                                                                <img src="https://www.judicial.gov.tw/work/work22/animation/3_4_2.gif" alt="">
                                                              </span>
                                                              <span class="text">什麼是勞動事件法</span>
                                                            </a>
                                                          </li>
                                                          <li>
                                                            <a href="">
                                                              <span class="pic">
                                                                <img src="https://www.judicial.gov.tw/work/work22/animation/3_4_1.gif" alt="">
                                                              </span>
                                                              <span class="text">勞動事件法</span>
                                                            </a>
                                                          </li>
                                                        </ul>
                                                      </div>
                                                    </div>
                                                  </div>
                                                  <div class="reform_link"><a href="#">司法院與行政院司法改革進度追蹤資訊平台</a></div>
                                                </div>
                                                <h3 class="tabItem"><a href="#">憲法訴訟法</a></h3>
                                                <div class="tabContent">
                                                  <h4>只有一張圖就沒有ul li</h4>
                                                  <div class="reform_date">憲法訴訟法 推動中</div>
                                                  <div class="content">
                                                    <div class="reform_pic left">
                                                      <a href="">
                                                        <span class="reform_caption">讓完全沒有審判專業知識、</span>
                                                        <img src="images/reform.jpg" alt="">
                                                      </a>
                                                    </div>
                                                    <div class="right_block">
                                                      <div class="block">
                                                        <!-- 只有一張圖就沒有ul li -->
                                                        <a href="">
                                                          <span class="pic">
                                                            <img src="https://via.placeholder.com/97x256.png" alt="">
                                                          </span>
                                                          <span class="text">什麼是勞動事件法</span>
                                                        </a>
                                                      </div>
                                                    </div>
                                                  </div>
                                                  <div class="reform_link"><a href="#">司法院與行政院司法改革進度追蹤資訊平台</a></div>
                                                </div>
                                                <h3 class="tabItem"><a href="#">終審法院大法庭制度</a></h3>
                                                <div class="tabContent">
                                                  <h4>終審法院大法庭制度</h4>
                                                  <div class="reform_date">終審法院大法庭制度 推動中</div>
                                                  <div class="content">
                                                    <div class="reform_pic">
                                                      <a href="">
                                                        <span class="reform_caption">讓完全沒有審判專業知識、經驗的一般國民加入審判程序，參與聽訟、問案及最後判決形成的過程，使司法審判更透明...</span>
                                                        <img src="images/reform.jpg" alt="">
                                                      </a>
                                                    </div>
                                                  </div>
                                                  <div class="reform_link"><a href="#">司法院與行政院司法改革進度追蹤資訊平台</a></div>
                                                </div>
                                                <h3 class="tabItem"><a href="#">終審法院大法庭制度</a></h3>
                                                <div class="tabContent">
                                                  <h4>終審法院大法庭制度</h4>
                                                  <div class="reform_date">終審法院大法庭制度 推動中</div>
                                                  <div class="content">
                                                    <div class="reform_pic">
                                                      <a href="">
                                                        <span class="reform_caption">讓完全沒有審判專業知識、經驗的一般國民加入審判程序，參與聽訟、問案及最後判決形成的過程，使司法審判更透明...</span>
                                                        <img src="images/reform.jpg" alt="">
                                                      </a>
                                                    </div>
                                                  </div>
                                                  <div class="reform_link"><a href="#">司法院與行政院司法改革進度追蹤資訊平台</a></div>
                                                </div>
                                              </section>
                                              <!-- <div class="reform_more"><a href="">看更多</a></div> -->
                                            </div>
                                          </div>
                                        </section>
                                        <!-- 司法新視野 -->
                                        <section class="vision_block">
                                          <h2 class="blkTitle">司法新視野<span>New Vision</span></h2>
                                          <div class="weekly">
                                            <a href="">
                                              <span class="date">108.05.31</span>
                                              <h3>司法周刊<span>Judicial Weekly</span></h3>
                                              <span class="vol"><em>1945</em>期</span>
                                              <p>肇事逃逸罪「肇事」之範圍部分違法律明確性原則失效肇事逃逸罪「肇事」之範圍部分違法律明確性原則失效</p>
                                            </a>
                                          </div>
                                          <div class="fordummies">
                                            <div class="pic">
                                              <img src="images/fordummies.jpg" alt="">
                                            </div>  
                                            <h3>圖說司法</h3>
                                            <p>提供司法圖文及影音等資訊，提升民眾對司法的認知提供司法圖文及影音等資訊</p>
                                            <div class="more"><a href="#">更多</a></div>
                                          </div>
                                          <div class="video">
                                            <div class="pic">
                                              <img src="images/video.jpg" alt="">
                                            </div>  
                                            <h3>司法影音</h3>
                                            <p>以「司法便民‧禮民」為出發點，提供最新司法影片為出發點，提供最新司法影片</p>
                                            <div class="more"><a href="#">更多</a></div>
                                          </div>
                                        </section>

                                        <section class="epaper_block">
                                          <form action="" class="form_inline">
                                            <div class="form_grp">
                                              <label for="mustSameAsId">訂閱司法電子報</label>
                                              <!-- <input name="username" id="mustSameAsId" type="text" placeholder="請輸入電子郵件信箱" accesskey="S"> -->
                                              <span>本院提供司法新聞相關資訊之訂閱服務</span>  
                                              <input name="" type="submit" title="去訂閱" value="去訂閱" />
                                            </div>
                                          </form>  
                                        </section>

                                        <section class="statistics_block">
                                          <h2 class="blkTitle">司法統計<span>Judicial Statistics</span></h2>
                                          <div class="statistics_slider">
                                            <div class="img-container">
                                              <a href="#" title="">
                                                <img src="images/statistics.jpg" alt="" class="cover">
                                              </a>
                                            </div>
                                            <div class="img-container">
                                              <a href="#" title="">
                                                <img src="images/statistics.jpg" alt="" class="cover">
                                              </a>
                                            </div>
                                            <div class="img-container">
                                              <a href="#" title="">
                                                <img src="images/statistics.jpg" alt="" class="cover">
                                              </a>
                                            </div>
                                          </div>
                                        </section>

                                      </div>
                                    </div>
                                    <!-- fatfooter Start -->
                                    <?php require_once('include/fatfooter.html'); ?>
                                    <!-- fatfooter End -->
                                    <!-- footer Start-->
                                    <?php require_once('include/footer.html'); ?>
                                    <!-- footer End -->
                                  </div>
                                  <?php require_once('include/js.html'); ?>

                                </body>

                                </html>